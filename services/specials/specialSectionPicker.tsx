...
/**
 * Function that returns the Elements for the Section to the Special Area.
 * @param {any} item with the data of the section.
 * @param {number} index with the index of the item in the page.
 * @return {TSX.Element}: The TSX code with the Section Elements.
 */
export default function specialSectionPicker(item: any, index: number) {
  const backgroundLogic =
    isOddOrEven(index) == 0 ? "" : theme.colors.oddSection;
  const sectionFormat =
    item.section_type == "side_by_side_texts" ? (
      ...
      <Box
        sx={{
          width: "100%",
          backgroundColor: backgroundLogic,
          paddingTop: 0.4,
          paddingBottom: 8,
        }}
      >
